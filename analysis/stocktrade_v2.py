...
def get_stock_hist_data(code=None,start=None,end=None,sh_df=None,sz_df=None,cyb_df=None,table_name=None):
    ...
    param['endtime']=end
    try:
         df_db= pd.read_sql_query('SELECT * FROM db_test.tick_data_total_v where code=%(code)s and date>=%(begintime)s and date<=%(endtime)s',sql.engine,params =param)
    except Exception,e:
         df_db = None
...
def  get_handler_stock_fs_data(code = None,date=None):
     ...
     record_ = [
         [sale_v, buy_v, diff_v, s_p, b_p, dif_p, sale_m, buy_m, diff_m, s_m_p, b_m_p, dif_m_p, dd_sale_v, dd_buy_v,
          dd_diff_v, dd_total_v, dd_sale_m, dd_buy_m, dd_diff_m, dd_total_m, dd_diff_m_var]]
     fs_df = pd.DataFrame(record_, columns=fs_colums, index=[date])
    #开始取数据
...
def get_stock_hist_data_batch(code_list = None,start=None,end=None,sh_df=None,sz_df=None,cyb_df=None,table_name=None):
    result =[]
    for code in code_list:
        try:
            ...
            if df is not None:
                ...
                res = tp.save_db(data=df,table_name=table_name,con=sql.engine,flag='append')
            result.append(res)
        except Exception,e:
            log.error(code)
            log.error(sys.argv)
...
if __name__=='__main__':
    ...
    for i in range(batch+1):
        ...
        pool.apply_async(get_stock_hist_data_batch,(batch_data,start,end,sh_df,sz_df,cyb_df,table_name,))
